...
public class FullStayArea : MonoBehaviour
{
    ...
    private bool isAreaOpen = false; // 清理区是否处于开启状态
    ...
    {
        Debug.Log("FullStayArea: 播放开启动画");
        isAreaOpen = true;

        // TODO: 在这里添加开启动画代码
        // 例如：播放Spine动画、缩放动画、透明度动画等
    ...
    public int AddBubbles(List<BubbleItem> bubbles)
    {
        ...
        // 🎯 检查是否需要播放开启动画
        // 条件：清理区原本是关闭状态 && 要放入的泡泡大于1个
        bool shouldPlayOpenAnimation = !isAreaOpen && bubblesToStore > 0;

        if (shouldPlayOpenAnimation)
        {
            Debug.Log($"FullStayArea: 满足开启动画条件 - 原本关闭状态且要放入{bubblesToStore}个泡泡");
            PlayOpenAnimation();
        }
    ...
    public BubbleItem RemoveBubbleAt(int index)
    {
        ...
        // 🎯 检查是否需要播放关闭动画
        // 条件：移出后清理区变为空 && 清理区原本是开启状态
        if (storedBubbles.Count == 0 && isAreaOpen)
        {
            Debug.Log("FullStayArea: 满足关闭动画条件 - 移出了最后一个物品");
            PlayCloseAnimation();
        }
    ...
    public void ClearStorage()
    {
        ...
        if (wasOpen)
        {
            Debug.Log("FullStayArea: 清空存储区域，播放关闭动画");
            PlayCloseAnimation();
        }
